...
func (d *Doc) Replace(value any) {
	d.v = assertValueType(value)
	d.setBack()
}

func (d *Doc) Set(key Key, value any) *Doc {
...
func (d *Doc) SetPath(path []Key, value any) {
	if len(path) == 0 {
		return
	}
...
func (d *Doc) Insert(index int64, value any) {
	value = assertValueType(value)
	a, err := d.CastArr()
	if err != nil {
		return
...
func (d *Doc) Remove(value any) int64 {
	a, err := d.CastArr()
	if err != nil {
		return -1
	}
...
func (d *Doc) RemoveAll(value any) {
	a, err := d.CastArr()
	if err != nil {
		return
	}
...
func (d *Doc) preAlloc(key Key) {
	...
	switch key.(type) {
	case string:
		d.v = make(map[string]any)
	case int64:
		d.v = make([]any, 0)
	}
}

func (d *Doc) setArr(key int64, value any) {
	a, err := d.CastArr()
	if err != nil {
		return
	}
...
func (d *Doc) setMap(key string, value any) {
	m, err := d.CastMap()
	if err != nil {
		return
	}
...
func (d *Doc) CastMap() (map[string]any, error) {
	m, ok := d.v.(map[string]any)
	if !ok {
		return nil, errCastFailed("map[string]any")
	}
	return m, nil
}

func (d *Doc) CastArr() ([]any, error) {
	a, ok := d.v.([]any)
	if !ok {
		return nil, errCastFailed("[]any")
	}
	return a, nil
}
...
func (d *Doc) Map(df ...map[string]any) map[string]any {
	o, err := d.CastMap()
	if err != nil && len(df) > 0 {
		o = df[0]
	}
...
func assertValueType(v any) any {
	switch v2 := v.(type) {
	case int8, int16, int32, int64, int,
		uint, uint8, uint16, uint32, uint64,
		float32, float64,
...
func errInvalidValue(value any) error {
